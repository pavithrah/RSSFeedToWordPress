...
def generate(filename, SITE_URL):
    ...
    for item in reversed(get(root, "//%(ns)sentry")):
        ...
        if not content:
            ...
            if len(content) > 0:                
                summary = content[0].replace("&lt;", "<").replace("&gt;", ">")
